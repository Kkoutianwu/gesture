...
int main(int argc, char *argv[])
{
	...
	//3、打开骨骼跟踪事件
	hr = NuiSkeletonTrackingEnable(skeletonEvent, NUI_SKELETON_TRACKING_FLAG_ENABLE_SEATED_SUPPORT);//半身模式
	if (FAILED(hr))
	{
					...
					skeletonFrame.SkeletonData[i].eSkeletonPositionTrackingState[NUI_SKELETON_POSITION_SHOULDER_CENTER] != NUI_SKELETON_POSITION_NOT_TRACKED)
				{
					...
					if (count > 3)
					{
						...
						/*		cout << "setoff1 : " << "(" << -(skeletonFrame.SkeletonData[i].SkeletonPositions[11].z - dsetoff[2]) * 100 << ", " << -(skeletonFrame.SkeletonData[i].SkeletonPositions[11].x - dsetoff[0]) * 100 << ", " << (skeletonFrame.SkeletonData[i].SkeletonPositions[11].y - dsetoff[1]) * 100 << ")" << endl;
							...
							//cout << skeletonFrame.SkeletonData[i].SkeletonPositions[7].y << ", " << skeletonFrame.SkeletonData[i].SkeletonPositions[4].y;
							//cout << "setoff1 : " << "(" <<  dsetoff[2] << ", " <<  dsetoff[0]<< ", " << dsetoff[1]<< ")" << endl;
						...
						dsetoff[0] = skeletonFrame.SkeletonData[i].SkeletonPositions[11].x;
						dsetoff[1] = skeletonFrame.SkeletonData[i].SkeletonPositions[11].y;
						dsetoff[2] = skeletonFrame.SkeletonData[i].SkeletonPositions[11].z;
					}
